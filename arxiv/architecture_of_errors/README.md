...
- **Phase 9 (final surgical fixes — technical-seam closure)**: a set of 12 surgical edits + bonus closing sentence that close the corpus-vs-sequence ambiguity in Postulate 1 / Proposition 1. Headline changes: (a) §3.2 introduces a **Notation block** distinguishing $T$ (corpus discovery), $h(n) = \beta k(n)$ (per-sequence exposure), and the three catalogues $C_D$, $C_{\text{seen},D}(T)$, $C_{\text{active},D}(n)$; (b) **Postulate 1** is restated as "Patch-indexed catalogue discovery" over $T$, separating it from per-sequence scaling; (c) a new **Assumption 2** ("Per-sequence active-mode exposure") supplies the bound over $h(n)$ that Proposition 1 actually uses; (d) **Proposition 1** is now parametric in $C_{\text{eff}}$ — either $C_{\text{active},D}(n)$ for per-sequence scaling or $C_D$ for full domain-library deployment — with two cleanly separated regimes; (e) §3.6 sensitivity table is replaced with **symbolic-rates form** (logarithmic, Heaps, saturating), avoiding the corpus-vs-sequence-units trap; (f) §1.2 "Heaps makes it hopeless" sentence is removed (internal contradiction with §3.5); (g) "erases" outside Pattern A is replaced with mechanism-appropriate language ("removes the execution-error component," "strongly reduces while leaving residual failures in"); (h) §5 "50 interventions" framing softened to "planning prior, not universal constant"; (i) §7 conclusion qualified to "patch-indexed and should be estimated from local discovery curves"; (j) closing sentence at end of §3.2 names what each of $T$, $h(n)$, and $|C_D|$ measures. Build still 24 pages.
- **Phase 8 (domain-patch refinement)**: a surgical addition introducing the manifold-theoretic notion of a *domain patch* $P_D$ — a bounded sub-region of Part 1's stratified manifold where real deployment lives. The patch caps the available mode catalogue $|C_D|$. The composed bound becomes $|C_{\text{observed}}(t)| \leq \min(A_D + \sigma_D \ln t, |C_D|)$. The logarithmic discovery rate inside the patch is unchanged; the cap is the new claim. Postulate 1's inequality is rewritten in patch-indexed form; Proposition 1 picks up a "cap regime" note (in that regime, the library size is asymptotically independent of $n$); §5 reframes "intervention library" as "local patch coverage" with cardiology/legal/code examples; §6.3 adds a "patch-shift between deployments" caveat. Capacity and plasticity material from the parallel `capplas_*` harvests is intentionally left out per user direction; the patch is anchored manifold-theoretically (topology / differential geometry), not operationally (no k-NN / LongPPL / HΔH footnote).
