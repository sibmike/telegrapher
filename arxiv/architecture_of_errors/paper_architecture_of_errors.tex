...
intervention categories from prior work, plus 28 GOLD-tier
capability-elimination citations across six axes, are consistent with a
small library on the order of tens of interventions covering the head of
the failure distribution in the per-hard-token regime. We test the
polylog conclusion under four cluster-count laws (logarithmic discovery,
Heaps at \(b \in \{0.3, 0.5, 0.6\}\), saturating exponential) and find
...
(Dziri et al., 2023; Kuratov et al., 2024; METR, 2025), re-audited here,
decays over variables other than raw token length --- compositional
graph size, fact count, log-time horizon. This relocates rather than
dissolves the practical worry: the framework identifies which
interventions help (capability provisioning along the actual decay axis)
and which do not, but does not claim the regimes where
...
reporting 100\% elimination on RAGTruth (Acurai authors, 2024; 95\% CI
91--100\%, benchmark-conditional --- not a by-construction guarantee for
RAG in general); over-refusals to preference optimisation (Karaman et
al., 2024); tool-call errors to structured uncertainty (Suri et al.,
2025). We collect twelve such categories with quantitative ``one
...
MWPES, the mode-rate parameter falls in the range
\(\sigma \in [0.87, 1.85]\), with \(\sigma \approx 1.85\) adopted from
the ErrorAtlas anchor as a conservative target. It proves a polylog
bound on the per-hard-token intervention library size,
\(m \geq \lceil |C|^{1 - \varepsilon / e_{\text{hard}}} \rceil\), and
shows that under \(k \sim \log n\) the budget grows as
\(O\big((\log\log n)^{c}\big)\) with \(c < 1\); the
\emph{sequence}-level analogue is strictly tighter and approaches
full-catalogue coverage as \(k\) grows. It synthesises evidence for the
three load-bearing claims --- clustering (Section 4.1),
cluster-selective capability interventions (Section 4.2), and sublinear
length scaling (Section 4.3) --- drawing on approximately sixty prior
published results, including a six-axis capability-elimination harvest
with 28 GOLD-tier citations. And it re-audits the most-cited steep-decay
results (Dziri et al., 2023; Kuratov et al., 2024; METR, 2025; Wan et
al., 2025), showing that each decays over a variable other than raw
...
17-category taxonomy, is therefore a conservative input for the
engineering question. When interventions cover \emph{sets} of L2
categories, the formal coverage problem becomes weighted set cover (with
weights given by category mass); Proposition 1's ranked-category form is
a tractable proxy for that problem, and the practical capability library
required to hit a given coverage target is generally smaller than the
formula's \(m\). We do not claim the polylog conclusion holds \emph{a
fortiori} in a formal-optimality sense --- that would require a
set-cover argument we do not develop here. We claim only that the
ranked-category bound is a conservative approximation to the set-cover
quantity that matters in practice.
...
\(m \geq 2\) and \(|C| \geq 2\), the range where the ErrorAtlas top-5 /
top-10 anchors sit. We adopt the empirical conventions
...
\] As \(\varepsilon_{\text{seq}}\) shrinks toward zero,
\(\tau_{\text{seq}} \to 0\) and the exponent approaches one, so \(m\)
approaches \(|C_{\text{eff}}|\) --- essentially every named mode must be
covered.
...
\(m\) approaches \(|C|\). The widely-quoted ``fix \(\approx 50\)
patterns per domain → \(\approx 90\%\) error reduction'' claim is best
understood as a per-hard-token statement; the analogous sequence-level
claim requires correspondingly more interventions and tighter coverage
of the tail. Production systems with one-shot reliability targets should
plan for the stricter regime --- and check whether they are in Regime
(i), where catalogue-budgeting cannot help at all.
...
cluster-based harvest below) yields \textbf{28 GOLD-tier citations}
...
learned verifiers, classifier rerouting, process supervision). Each axis
is independently confirmed by 3--9 GOLD citations. The 28 citations
exhibit three structural patterns of capability-elimination that
strengthen --- and refine --- the cluster-orthogonality claim.
...
The 28 GOLD citations, organised by axis and pattern, are summarised
below.
...
→ 9\% & C \\
Arithmetic & MedRaC (Wang Z. et al., 2025) & Arithmetic in medical
...
stacking interventions targeting orthogonal failure modes can produce
large compound reliability gains. In their parallel-consensus framework,
...
voting. AgentSquare (AgentSquare authors, 2024) similarly shows
supra-additive gains when modules target distinct failure modes. These
are encouraging but benchmark- and aggregation-specific.
...
family of models they evaluate (spanning sub-billion to 175B-parameter
variants). This is direct empirical evidence against per-step
...
Seven of the 28 GOLD citations in Section 4.2.0 --- DeepSeek-Prover-V2's
Lean proof checker, OpenAI Structured Outputs, ToolDec, DINGO, XGrammar,
XGrammar-2, and LlmFix's static syntax check --- achieve residual error
rate equal to zero \emph{by mathematical construction}, not
statistically. Constrained decoders set \(P(\text{invalid token}) = 0\)
at every generation step, so no invalid token can be emitted. Proof
kernels reject any output that fails type-checking. Schema validators
reject any output missing a required field. (Acurai's RAG result is
excluded from Pattern A here, despite its 100\% RAGTruth number; it is a
...
Pattern B. LlmFix's \texttt{NameError} repair is also excluded, since
its residual is 2.3\% rather than zero --- Pattern C.)
