...
by category mass); Proposition 2's ranked-category form is a tractable
...
Proposition 2: once enough failures have been sampled that
...
evidence levels. Appendix A.5 shows that the qualitative polylog
conclusion of Proposition 2 survives across all four candidate
...
dictionary can guarantee residual error below \(\varepsilon\) over all
of \(D\).}
...
\textbf{Proposition 2 (Patch-Local Intervention Budget).} \emph{Fix a
deployment patch \(D\). Let \(\varepsilon \in (0, e_{\text{hard}})\) be
the target per-hard-token residual error rate. Under the log-coverage
approximation of §3.3, a library covering the dominant local modes
satisfies the target once}
...
decisions and \(k(n) = \Theta(\log n)\), then the pre-cap intervention
budget grows doubly-logarithmically in sequence length. Once the patch
catalogue saturates, the required budget becomes domain-constant in
\(|C_D|\).}
...
A common engineering rule of thumb --- ``fix ≈50 patterns per domain →
≈90\% error reduction'' --- is best understood as a per-hard-token
planning prior. The analogous sequence-level claim requires
...
The conditional consequence (Proposition 2) is that the intervention
...
doubly-logarithmic rate; the conservative Heaps variant of Appendix A.5
gives \((\log n)^b\) with small \(b\); the symbolic-rates form of
Appendix A.5 shows the polylog conclusion is stable across this family.
...
\textbf{Step 5: no finite dictionary covers the domain.} Suppose, again
for contradiction, that some finite intervention dictionary
\(\mathcal{I}\) covers all of \(D\). Then \(\mathcal{I}\) covers every
finite prefix \(\{f_1, \ldots, f_{j-1}\}\) for every \(j\). By
intervention-unboundedness, any dictionary covering that prefix fails to
cover \(f_j\). Therefore \(\mathcal{I}\) does not cover \(f_j\),
contradicting the assumption that \(\mathcal{I}\) covers all of \(D\).
\(\square\)

\subsubsection{A.2 Derivation of Proposition 2: Patch-Local Intervention
Budget}\label{a.2-derivation-of-proposition-2-patch-local-intervention-budget}

\textbf{Conditions used.} Proposition 2 depends on four assumptions:
...
\item
  ...
  \(F(m; |C|) = \min(1,\, \ln m / \ln |C|)\) on the declared domain
  \(m \geq 2\), \(|C| \geq 2\).
\item
...
\(\tau_{\text{seq}}\) gives the required library size
...
output. This is why the main paper treats the ``≈50 patterns'' rule of
thumb as a per-hard-token planning prior, not a one-shot sequence-level
SLA.
...
stated head-heavy form, \emph{then} the required per-hard-token
