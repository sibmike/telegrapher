...
Memory, and Policy with a semantic-merge orchestrator. SkillRL uses RL
reward distillation with a hard threshold at task-category accuracy
below 0.4. AlphaEvolve runs a Gemini-driven ensemble with a cascade
evaluator and a MAP-Elites archive, deployed at Google to gain +0.7\%
Borg compute worldwide, +23\% on the Gemini training kernel, +32.5\% on
...
multiplication (AlphaTensor had already found a 47-multiplication
algorithm over GF(2); AlphaEvolve's contribution is the first
improvement that holds in general fields). DGM modifies its own source
code with archive-based parent selection and lifts SWE-bench from 20\%
to 50\%. The cluster maximises velocity. Enterprise governance is
uniformly absent.
