...
deployed systems have closed which of those five steps; the missing
architecture of §9 names the closure that no public system has yet
achieved.
...
convergence proposition of the loss to a \(G\)-stable scaffold, expands
the six-operation mechanism stack, gives the full optimisation-analogy
correspondence, and works through the RAG, pitch-review, tool-use, and
memory cases that make the mechanism visible.
...
agent and edits it with explicit deep-learning-style controls: rollout
batches, reflection minibatches, add/delete/replace edits, a textual
learning-rate budget with cosine decay, a held-out selection-split
accept gate, a rejected-edit buffer that retains failed proposals as
negative feedback, and an epoch-wise slow/meta update for longer-horizon
consolidation. SkillOpt's introduction and method make the same
operational analogy explicit (parameter → skill document; gradient →
trajectory-derived edit direction; learning rate → edit budget;
validation check → held-out selection gate; stable training setting →
batch / minibatch / schedule / gate), and the framing --- \emph{skill
optimization as deep-learning-style optimization over an external
natural-language state} --- is the same disciplined gradient-like update
mechanism for the scaffold layer that this paper argues is missing in
current production.
