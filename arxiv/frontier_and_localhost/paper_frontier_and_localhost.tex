...
conceptual and survey-based. A practitioner-facing companion
(ScaffoldOps) handles reference architectures, maturity ladders, design
principles, and the full failure taxonomy.
...
and eight design principles --- is the subject of a companion paper,
\emph{ScaffoldOps: Governance Patterns for Patch-Plastic LLM Systems.}
This paper is the survey and the framework. ScaffoldOps is the operating
manual.
...
most of (1)--(5) but rarely (6). ScaffoldOps (forthcoming companion)
walks the implementation patterns; this paper's purpose is to make the
discipline visible.
...
2.} The maturity-rubric details, score distributions, and per-substrate
exemplars are covered in ScaffoldOps §3.
...
evidence carry highest weight in the cluster analysis (§6); T3 systems
...
\subsection{6. Convergent patterns}\label{convergent-patterns}
...
\subsection{7. The two-loop design
...
averaging across user/project shards, gated by human PR review. The full
ML-lens walk-through is in ScaffoldOps §8. For Paper 3's purposes, the
key observation is that OpenCore demonstrates \emph{cross-organisation}
...
\subsection{8. The empty corner}\label{the-empty-corner}
...
promoted artifact. ScaffoldOps §10 sketches the eight safety
requirements an implementor would need; this paper's claim is only that
the empty corner is \emph{named}. It is no longer ``no system closes
both loops'' --- too coarse --- but a specific four-property combination
that the survey can audit any future system against.
...
\subsection{9. A new failure surface}\label{a-new-failure-surface}
...
severity ratings and counter-principles P1--P8) is in ScaffoldOps
§§6--7.
...
2024--2026 work, and largely unmitigated in production. ScaffoldOps
walks the mitigation principles in detail.
...
Several open questions remain. \emph{Failure-triggered memory} is rare
in production despite being the most obvious Loop-1 signal; the surveyed
memory systems prefer to record preferences and successful facts rather
than residuals. \emph{Cross-customer skill promotion} is absent ---
AGENTS.md is cross-vendor but Anthropic's skills repository is one-pool;
no surveyed system has a vetted cross-organisational marketplace with
eval-gated promotion. \emph{Versioning of memory} is essentially
unsolved outside SSGM. \emph{Curriculum at the scaffold layer} ---
selecting which sessions inform the gradient --- has no surveyed
instance. \emph{Overfitting detection at Loop-2 promotion} --- a
held-out eval set per tenant that catches scaffolds over-fitting one
deployment before they propagate --- likewise. Each is a Paper-4 target.
