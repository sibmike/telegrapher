...
The central contribution is a shift in the reliability object: universal LLM reliability is not a finite-library problem, but patch-local reliability can be treated as catalogue discovery and intervention coverage. We formalise this transition with two propositions and one corollary. Proposition~\ref{prop:nounidict} gives the negative result: if an unbounded domain keeps producing intervention-distinguishable failures, no finite intervention dictionary can guarantee bounded residual error universally. Corollary~\ref{cor:invdiscovery} gives the inverse economic implication of logarithmic discovery: linearly expanding the discovered-mode catalogue requires exponentially more observed hard-failure events, so open-domain tail discovery has rapidly diminishing returns. Proposition~\ref{prop:budget} gives the corresponding positive result inside a fixed deployment patch: under logarithmic or capped mode discovery, the intervention budget required to bound per-hard-token residual error satisfies $m \geq \lceil |C_{\text{eff}}|^{1 - \varepsilon / e_{\text{hard}}} \rceil$, with a doubly-logarithmic sequence-length rate as the optimistic pre-cap special case and a domain-constant budget once the patch catalogue saturates. The sequence-level analogue is strictly tighter and approaches full-catalogue coverage as $k$ grows; we surface that asymmetry rather than bury it.
...
The logarithmic discovery postulate inverts to a sample-budget cost on distinct-mode discovery. If $q(T) \approx A_D + \sigma_D \ln T$ with $\sigma_D > 0$, then discovering $q$ distinct modes requires
\begin{equation}\label{eq:invcost}
T(q) \;\approx\; \exp\!\left(\frac{q - A_D}{\sigma_D}\right)
\end{equation}
observed hard-failure events, and each additional $\Delta q$ modes raises the sample budget by a factor of $\exp(\Delta q / \sigma_D)$.
\end{corollary}

\paragraph{Engineering meaning.}
Linearly expanding the discovered-mode catalogue therefore requires multiplicatively more data. At the conservative calibration $\sigma_D \teApprox 1.85$, five extra discovered modes need roughly $15\times$ more observed hard failures; ten extra modes need roughly $220\times$ more. The claim is about \emph{newly distinguishable} tail modes, not ordinary failures, which may remain common. Combined with the head-heavy coverage model of \S\ref{sec:coverage}, this gives the economic interpretation: generic open-domain training spends increasing data on low-mass tail discovery, while patch-local reliability directly targets the high-mass local head. Full derivation, the Heaps polynomial variant, and a mode-mediated capability-gain sub-corollary are in Appendix~\ref{app:invdiscovery}.
...
\end{equation}
as a \emph{postulated} parametric approximation to that cumulative top-$m$ mass, not as a raw count of categories covered. Reading the formula: the first few interventions cover disproportionate mass (the head of the distribution is dense), then incremental coverage falls off as the library saturates the catalogue. For our ErrorAtlas anchor ($|C|=17, m=5$), $F(5;17) \teApprox 56.8\%$; doubling the library to $m=10$ pushes it to $F(10;17) \teApprox 81.3\%$.
...
\begin{proposition}[Patch-Local Intervention Budget]\label{prop:budget}
Fix a deployment patch $D$. Let $\varepsilon \in (0, e_{\text{hard}})$ be the target per-hard-token residual error rate. Under the log-coverage approximation of \S\ref{sec:coverage}, a library covering the dominant local modes satisfies the target once
...
A common engineering rule of thumb, ``fix $\teApprox 50$ patterns per domain $\teArrow \teApprox 90\%$ error reduction,'' is best understood as a per-hard-token planning prior. The analogous sequence-level claim requires correspondingly more interventions and tighter coverage of the tail.
...
\paragraph{Step 5: no finite dictionary covers the domain.}
Suppose, again for contradiction, that some finite intervention dictionary $\mathcal{I}$ covers all of $D$. Then $\mathcal{I}$ covers every finite prefix $\{f_1, \ldots, f_{j-1}\}$ for every $j$. By intervention-unboundedness, any dictionary covering that prefix fails to cover $f_j$. Therefore $\mathcal{I}$ does not cover $f_j$, contradicting the assumption that $\mathcal{I}$ covers all of $D$. \qed

\subsection{Derivation of Proposition~\ref{prop:budget}: Patch-Local Intervention Budget}
\label{app:proof-budget}

\paragraph{Conditions used.}
Proposition~\ref{prop:budget} depends on four assumptions:
\begin{enumerate}[leftmargin=1.8em,itemsep=0.25ex]
\item \textbf{Coverage model.} The cumulative hard-error mass covered by the top $m$ modes is approximated by
\[
F(m; |C|) \;=\; \min\!\left(1,\ \frac{\ln m}{\ln |C|}\right)
\]
on the declared domain $m \geq 2$, $|C| \geq 2$.
...
Per-hard-token reliability is easier than sequence-level reliability. A library that gives a large reduction in residual hard-token error may still be insufficient for one-shot sequence-level guarantees when many hard decisions occur in a single output. This is why the main paper treats the ``$\teApprox 50$ patterns'' rule of thumb as a per-hard-token planning prior, not a one-shot sequence-level SLA.
...
Proposition~\ref{prop:budget} is conditional engineering math. It does not prove that LLM failures universally obey logarithmic mode discovery. It proves that \emph{if} a bounded patch has a finite or effectively capped reachable catalogue, \emph{and if} cumulative intervention coverage follows the stated head-heavy form, \emph{then} the required per-hard-token intervention budget grows slowly and becomes constant after catalogue saturation.
...
The body Corollary~\ref{cor:invdiscovery} inverts the logarithmic discovery postulate to read out a sample-budget cost for distinct-mode discovery. This appendix gives the algebra, the numerical anchors, sensitivity to the Heaps cluster-count law of \S\ref{app:heaps}, and a separate mode-mediated gain corollary that connects discovery cost to broad capability proxies.

\paragraph{Setup.}
Let $q(T) = |C_{\text{seen},D}(T)|$ be the number of distinct failure modes discovered in patch $D$ after $T$ observed hard-failure events. Postulate~\ref{post:logmode} approximates discovery as
\[
q(T) \;\approx\; A_D + \sigma_D \ln T, \qquad \sigma_D > 0.
\]

\paragraph{Inversion.}
Subtract $A_D$, divide by $\sigma_D$, and exponentiate. The sample budget required to discover at least $q$ distinct modes is
\[
T(q) \;\approx\; \exp\!\left(\frac{q - A_D}{\sigma_D}\right).
\]

\paragraph{Multiplicative cost per additional mode.}
Comparing the budget at $q + \Delta q$ to the budget at $q$,
\[
\frac{T(q + \Delta q)}{T(q)} \;\approx\; \exp\!\left(\frac{\Delta q}{\sigma_D}\right),
\]
so each block of $\Delta q$ additional modes costs a \emph{multiplicative} sample increase. At the conservative calibration $\sigma_D \teApprox 1.85$ (\S\ref{sec:postulate}), $\exp(5/1.85) \teApprox 14.9$ and $\exp(10/1.85) \teApprox 222$: five extra modes cost roughly $15\times$ more observed hard failures, ten extra modes cost roughly $220\times$.
...
Under the Heaps cluster-count law of \S\ref{app:heaps}, $q(T) = K T^b$ with $b \in (0,1)$, the inverse cost is polynomial:
\[
T(q) \;=\; (q/K)^{1/b}.
\]
...
Once $q(T) \leq |C_D|$ has been saturated, discovery stops; the inversion applies only in the pre-cap regime. Inside saturated patches the corollary's multiplicative-cost claim is vacuous because no novel modes remain to discover, which is itself a property of the patch and not a failure of the corollary.

\paragraph{Mode-mediated capability gain (sub-corollary).}
Suppose broad capability or reliability gain $G$ inside the patch is approximately linear in the number of useful discovered modes, $G(q) = G_0 + \gamma q$ for some $\gamma > 0$. Composing with logarithmic mode discovery,
\[
...
\]
A linear gain in mode-mediated broad reliability therefore requires exponential growth in observed hard-failure exposure under the postulate. We deliberately phrase $G$ as a \emph{mode-mediated capability/reliability} proxy, not as ``intelligence'': the corollary does not say frontier scaling is useless or that intelligence requires exponential data in any general sense. It says, more narrowly, that for fixed deployment reliability where improvement is mediated by discovering new useful modes, generic open-domain training pays a heavy data tax relative to direct patch-local measurement and intervention.
