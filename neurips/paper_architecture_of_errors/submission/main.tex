...
The finite-catalogue claim is not universal. Before deriving the local intervention budget, we first state the corresponding negative result: if a domain keeps generating intervention-distinguishable failure modes, no finite dictionary can cover it. The positive result of this paper begins only after a deployment patch $D$ has been fixed.

\begin{proposition}[No Universal Finite Intervention Dictionary]\label{prop:nounidict}
Fix a residual-error tolerance $\varepsilon$. Say that an intervention \emph{covers} a failure mode if it reduces that mode's residual error below $\varepsilon$. A domain $D$ is \emph{intervention-unbounded} if it contains an infinite sequence of failures $(f_j)_{j=1}^{\infty}$ such that each $f_j$ is not covered by any finite intervention dictionary that covers $\{f_1, \ldots, f_{j-1}\}$. Then the $\varepsilon$-resolution failure catalogue $C_D^{\varepsilon}$ is infinite. Consequently, no finite intervention dictionary can guarantee residual error below $\varepsilon$ over all of $D$.
\end{proposition}

\begin{proof}
Suppose $C_D^{\varepsilon}$ were finite. Then after all its modes had appeared, every later failure would belong to one of the already-seen modes, so the interventions covering the earlier representative of that mode would also cover the later failure. But intervention-unboundedness says that each new $f_j$ is not covered by any dictionary covering the earlier failures, contradicting the assumption. Therefore $C_D^{\varepsilon}$ is infinite. Now suppose for contradiction that some finite dictionary $\mathcal{I}$ covered all of $D$. Then $\mathcal{I}$ in particular covers $\{f_1, \ldots, f_{j-1}\}$ for every $j$, so by intervention-unboundedness $\mathcal{I}$ does not cover $f_j$; but $\mathcal{I}$ covers all of $D$, so it covers $f_j$. Contradiction.
\end{proof}

\paragraph{Engineering interpretation.}
A universal reliability library is not a well-posed target. If the domain keeps changing the task family, tools, schemas, knowledge sources, and evaluator expectations, then new intervention-distinguishable failures can appear indefinitely. The correct reliability object is therefore not ``all LLM behaviour,'' but a bounded deployment patch. Proposition~\ref{prop:nounidict} is the inoculation against a misread of the rest of this paper: we are not claiming a fixed list of $\teApprox 50$ named patterns covers LLM use in general. We are claiming that \emph{inside a fixed operational patch} the reachable catalogue can be modelled as finite or effectively capped, and that is the regime in which the engineering math below applies.

\paragraph{From the negative to the positive result.}
Proposition~\ref{prop:nounidict} explains why universal long-context reliability is the wrong engineering object. The right object is a bounded deployment patch. Once $D$ is fixed, the question changes from ``can one library cover all possible LLM failures?'' to ``how large must the library be to cover the reachable catalogue $C_D$ inside this patch?'' We now compose the three ingredients of \S\S\ref{sec:postulate}--\ref{sec:coverage}, namely $\beta$-stratification (Definition~\ref{def:hard}), the active-mode bound (Assumption~\ref{ass:active}), and the log-coverage form (Eq.~\ref{eq:coverage}), into that local budget statement. For \emph{sequence-length scaling} we use Assumption~\ref{ass:active}, not Postulate~\ref{post:logmode}: the empirical sparsity $\alpha = k/n \in [0.05, 0.10]$ is a \emph{finite-length} statement, and under~\citet[\S3.1]{arbuzov2025beyond}'s $k \sim \log n$ regime, $\alpha$ decays as $O(\log n / n)$.

\paragraph{Conditionality.}
The bound below is engineering math, not a theorem about LLMs. It holds conditional on three things: (i)~the log-coverage form of \S\ref{sec:coverage} applied on its declared domain $m \geq 2$, $|C_{\text{eff}}| \geq 2$; (ii)~a residual target $\varepsilon \in (0, e_{\text{hard}})$ that is non-trivial but attainable; (iii)~Assumption~\ref{ass:active} for any sequence-length rate claim, with the patch hypothesis that a bounded operational patch justifies modelling the reachable catalogue as finite or effectively capped. A different cumulative fitted to the same ErrorAtlas/HumanEval/MWPES anchors would preserve the qualitative polylog conclusion (Appendix~\ref{app:heaps}) while changing the exponent.
...
\begin{description}[leftmargin=2em,itemsep=0.25ex]
  ...
  \]
  As the target tightens toward the feasibility boundary, $\tau_{\text{seq}} \to 0$ and $m \to |C_{\text{eff}}|$, so essentially every named mode must be covered.
